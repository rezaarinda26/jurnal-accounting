    ...
    <div class="py-10">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8 px-4 sm:px-0">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden p-8">
                <div class="mb-6">
                    <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Tambah PIC Baru</h2>
                    <p class="text-sm text-slate-500 mt-1">Daftarkan personil penanggung jawab baru ke sistem.</p>
                </div>
                
                <form method="POST" action="{{ route('pics.store') }}" class="space-y-6">
                    @csrf
                    
                    <div>
                        <x-input-label for="name" :value="__('Nama Lengkap PIC')" class="text-slate-600 dark:text-slate-300 font-medium" />
                        <input id="name" class="block mt-2 w-full border-slate-300 dark:border-slate-600 focus:border-primary-500 focus:ring-primary-500 dark:bg-slate-900 rounded-lg shadow-sm py-2.5" type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Ahmad Subardjo" required autofocus />
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="3" class="block mt-2 w-full border-slate-300 dark:border-slate-600 focus:border-primary-500 focus:ring-primary-500 dark:bg-slate-900 rounded-lg shadow-sm py-2.5" placeholder="Contoh: Divisi Operasional / Driver">{{ old('description') }}</textarea>
                    ...
                    <div class="pt-4 flex items-center justify-end gap-3 border-t border-slate-100 dark:border-slate-700">
                        ...
                        <button type="submit" class="px-6 py-2.5 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-lg transition-all shadow-sm shadow-primary-500/30 active:scale-95">Simpan PIC</button>
                    </div>
                </form>
